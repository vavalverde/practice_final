...
class FindEquipmentPage extends React.Component {
  ...
  renderPage() {
    ...
    const emails = _.pluck(EquipmentsInterests.collection.find({ interest: { $in: this.state.interests } }).fetch(), 'equipment');
    const equipmentData = _.uniq(emails).map(email => getEquipmentData(email));
    return (
      <div id="parent">
        <Container id="filter-page" style={{ paddingBottom: '35px' }}>
          ...
          <Card.Group style={{ paddingTop: '10px' }}>
            {_.map(equipmentData, (equipment, index) => {
